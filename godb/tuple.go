...
func (t *Tuple) writeTo(b *bytes.Buffer) error {
	...
	for _, field := range t.Fields {
		...
		case IntField:
			err := binary.Write(b, binary.LittleEndian, field.Value)
			if err != nil {
				return err
			}
...
func readTupleFrom(b *bytes.Buffer, desc *TupleDesc) (*Tuple, error) {
	...
	for _, field := range desc.Fields {
		...
		case IntType:
			...
			res.Fields = append(res.Fields, IntField{Value: intValue})
		case StringType:
			paddedValue := make([]byte, StringLength)
			err := binary.Read(b, binary.LittleEndian, &paddedValue)
			if err != nil {
				log.Fatal(err)
				return nil, err
			}
			strValue := string(paddedValue)
			strValue = strings.TrimRight(strValue, "\x00")
			res.Fields = append(res.Fields, StringField{Value: strValue})
		default:
...
func (t *Tuple) compareField(t2 *Tuple, field Expr) (orderByState, error) {
	...
	switch left.(type) {
	case IntField:
		leftInt := left.(IntField).Value
		rightInt := right.(IntField).Value
		if leftInt < rightInt {
	...
	case StringField:
		leftStr := left.(StringField).Value
		rightStr := right.(StringField).Value
		if leftStr < rightStr {
			return OrderedLessThan, nil
	...
	return OrderedEqual, fmt.Errorf("unsupported field type for comparison")
}
